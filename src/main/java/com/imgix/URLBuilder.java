...
public class URLBuilder {
  ...
  private static final Integer[] SRCSET_TARGET_WIDTHS = {
  ...
  public URLBuilder(String domain, boolean useHttps, String signKey, boolean includeLibraryParam) {
    ...
    if (domain == null || domain.length() == 0) {
      ...
      throw new IllegalArgumentException(
          "Domain must be passed in as a fully-qualified domain name and should not include a"
              + " protocol or any path element, i.e. \"example.imgix.net\".");
    }
  ...
  /**
   * This single-parameter constructor, accepts a domain string and returns a `URLBuilder` with it's
   * `useHttps` member variable set to `true` by default.
   *
   ...
   *
   * <p>This function creates a dpr based srcset if `params` contain either: - a width "w" param
   * _or_ a height "h" param.
   *
   * <p>Otherwise, a srcset of width-pairs is created.
   *
   ...
   *
   * <p>This function provides the ability to vary `tol` while generating a srcset over the default
   * range.
   *
   * @param path - path to the image, i.e. "image/file.png"
   * @param params - map of query parameters
  ...
  /**
   * Create a srcset given a `path`, map of `params`, `begin` and `end`.
   *
   * <p>This function provides the ability to create a srcset attribute over a custom range between
   * `begin` and `end` (inclusively) while using the default srcset width tolerance.
   *
   * @param path - path to the image, i.e. "image/file.png"
   * @param params - map of query parameters
  ...
  /**
   * Create a srcset given a `path`, map of `params`, `begin`, `end`, and `tol`.
   *
   * <p>This function provides the ability to create a completely custom srcset attribute. The
   * srcset width value range will `begin` and `end` on the specified values. The specified `tol`
   * determines the tolerable amount of width value variation.
   *
   * @param path - path to the image, i.e. "image/file.png"
   * @param params - map of query parameters
   * @param begin - beginning image width value
  ...
  public String createSrcSet(
      String path, Map<String, String> params, int begin, int end, double tol) {
    return createSrcSet(path, params, begin, end, tol, false);
  }

  /**
   * Create a srcset given a `path`, map of `params`, and the `disableVariableQuality` flag.
   *
   * <p>If `disableVariableQuality` is `false` then variable output is turned _on_. If
   * `disableVariableQuality` is `true` then variable quality output is turned _off_.
   *
   * @param path - path to the image, i.e. "image/file.png"
   * @param params - map of query parameters
   * @param disableVariableQuality - flag to toggle variable image output quality.
   * @return srcset attribute string
   */
  public String createSrcSet(
      String path, Map<String, String> params, boolean disableVariableQuality) {
    return createSrcSet(
        path, params, MIN_WIDTH, MAX_WIDTH, SRCSET_WIDTH_TOLERANCE, disableVariableQuality);
  }
  ...
  private String createSrcSetDPR(
      String path, Map<String, String> params, boolean disableVariableQuality) {
    StringBuilder srcset = new StringBuilder();
    Map<String, String> srcsetParams = new HashMap<String, String>(params);
   ...
   *
   * <p>If `begin`, `end`, and `tol` are the default values, then the targets are not custom, in
   * which case the default widths are returned.
   *
   * <p>A target width list of length one is valid: if `begin` == `end` then the list begins where
   * it ends.
   *
   * <p>When the `while` loop terminates, `begin` is greater than `end` (where `end` less than or
   * equal to `MAX_WIDTH`). This means that the most recently appended value may, or may not, be the
   * `end` value.
   *
   * <p>To be inclusive of the ending value, we check for this case and the value is added if
   * necessary.
   *
   * @param begin - beginning image width value
   * @param end - ending image width value
   ...
   * Create an `ArrayList` of integer target widths.
   *
   * <p>This function is the implementation details of `targetWidths`. This function exists to
   * provide a consistent interface for callers of `targetWidths`.
   *
   * <p>This function implements the syntax that fulfills the semantics of `targetWidths`. Meaning,
   * `begin`, `end`, and `tol` are to be whole integers, but computation requires `double`s. This
